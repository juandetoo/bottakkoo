...
+ 'FN:thiagonahuel\n' // Nama
+ 'ORG:Mr.Pato-Bot;\n' // Nama bot
+ 'TEL;type=CELL;type=VOICE;waid=59897407742:+59897407742\n' // Nomor bot
+ 'END:VCARD' 
...
Usa ${prefix}verify para verificarte y poder usar el bot.`
  ...
  ferr: 'perdón, ocurrio un error en mi sistema ❌',
  ...
  only: {
    ...
    admin: 'Tenes que ser admin para este comando',
    Badmin: 'tengo que ser admin para usar este comando',
				  ...
				  case 's':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('botako', '@thiagonahuel')} ${ran} -o ${ran}`, async (error) => {
